...
### how
@card: 怎麼挑題目最不後悔：先用風浪／膝蓋／交通成本／想看什麼做決策，把延伸日選成順、好撤退、少踩雷的版本。
...
### route
@card: 交通/移動通用原則：叫車、租車與火車的取捨都在這裡；先把移動方式決定好，延伸日就更像度假而不是轉乘硬仗。
...
### backup
@card: 延伸日常見 Plan B 開關：風、海象與岩岸安全；知道這些就能更果斷改題目，不把一天浪費在「硬撐不成立」的行程上。
...
### how
@card: 動線建議：只選 Sagres 或 Cabo 其一就回最穩；狀態好再做整天版。重點是看海的規模與風的存在感，不是跑景點清單。
...
### time
@card: 停留時間抓法：Sagres 1.5–3 小時、Cabo 30–60 分鐘；風大就縮短，把保暖與安全放第一，寧可快閃也不要久站硬撐。
- Sagres（堡壘＋海岸短走）：1.5–3 小時
- Cabo de São Vicente（燈塔/懸崖觀景）：30–60 分鐘（風大就更短）

### cost
@card: 花費結構很單純：交通＋Sagres 堡壘門票；不估死數字，先抓「會花在哪」與「值不值得加」就好，避免為了省小錢耗掉體力與時間。
...
### backup
@card: Plan B：風太大就只留觀景點短停、不久站；把它當平坦開闊散步，不要硬貼懸崖邊，把體感留在「震撼」而不是「被風吹到不爽」。
...
### how
@card: 動線建議：不自駕也能成立但時間要抓寬；先把 Silves（城堡＋老城）做主菜，再看天氣與交通決定要不要加 Monchique（把它當甜點）。
最穩（不自駕也成立）的排法：
...
### cost
@card: 花費/票務與體力成本：交通、門票與走路量的保守抓法都在這裡；先抓結構避免回程被拖崩，也更容易決定要不要加碼 Monchique。
...
### backup
@card: Plan B：陰天/起霧就把 Monchique 降級或砍掉，回到 Silves 主菜早回海邊休息；也可用酒莊/SPA/品酒當室內替代，把這天維持在舒服節奏。
...
### cost
@card: 花費結構：交通到 Portimão＋過河小船（若有）＋把預算留給河邊用餐；這天最值得花的通常不是門票，而是「坐下來吃一頓」。
以當天交通方案為準。保守抓法：
...
### backup
@card: Plan B：如果水上計程車/過河小船不確定，就把 Portimão 當主菜（河岸散步＋咖啡/博物館）照樣舒服；或直接叫車回 Lagos 省掉轉乘。
...
### how
@card: 交通與動線重點整理：先把 Lagos ↔ Faro 班次抓穩，再挑 2–3 個點慢慢走；想更度假才加潟湖段，不加也完全成立。
...
## ria-formosa | Ria Formosa 自然公園（潟湖船遊：最穩的水上放空）

一句話定位：如果你想「坐船看自然」但又怕外海顛簸，Ria Formosa 是阿爾加維最友善的選擇：看的是潟湖、沙洲與鳥，不是浪花與岩洞。
...
### how
@card: 玩法重點：從 Faro/Olhão/Tavira 碼頭上船、選 1–2 小時短程，把它當「潟湖散步」；期待值放在自然與安靜感，而不是洞穴與浪花。
用對玩法就會很舒服（省力版）：
...
### cost
@card: 花費抓法：不估死價格，先抓「付的是船＋導覽＋時長」；挑短程與碼頭上船通常更穩，也更符合放空日的節奏。
船型、時長、是否有導覽差異很大，建議不估死價格：抓概念是「付的是船與導覽」，而不是門票。

### backup
@card: Plan B：臨時沒訂到船，就把這天改成 Faro/Olhão 老城散步＋市場/咖啡的低壓力版本；重點是休息與補給，不必硬追水上活動。
...
### how
@card: 交通與動線重點整理：通常先到 Faro 再接下一段，回程別壓太晚；如果不想轉乘，直接改去 `faro` 會更輕鬆。
...
### cost
@card: 花費/票務與時間成本提醒：交通（多段轉乘）＋預留一餐慢慢坐下來吃；若加碼島上海灘，渡輪/水上計程車票價看季節與現場公告。
以官方查詢為準。保守抓法：
...
### how
@card: 最省力玩法：Lagos 叫車/自駕 10–15 分鐘到，主菜只做海濱長廊慢走＋找座位坐下來看海；狀態好再加一小段村內巷弄即可。
最省力玩法（直接照抄）：
...
### cost
@card: 花費結構很單純：交通＋餐飲；很適合把預算花在「坐下來吃一餐/喝一杯」與看海本身，幾乎不用門票成本。
幾乎不用門票：主要花費在交通與餐飲（很適合把預算花在「坐下來吃一餐」）。

### backup
@card: Plan B：風大就照走長廊但縮短時間，直接改成「咖啡/午餐＋看海」模式；Luz 的價值在平坦好撤退，不在於一定要走多遠。
...
### how
@card: 玩法定位：河口生態＋度假小鎮；半日照抄版是傍晚前到木棧道看潟湖、再回鎮上坐下來吃一餐，把它當「復健型散步」最對味。
...
### cost
@card: 花費結構很單純：交通＋餐飲；木棧道本身免費。把預算留給「坐下來吃一餐」會比追更多點更舒服。
...
### how
@card: 新規預期先建立：不能游泳進洞、多數情況不能登陸沙灘；玩法以小船導覽或上方俯瞰為主，進洞停留通常很短，把「抬頭看天窗」當主菜。
...
### comfort | 省力/舒適版：怎麼訂到「舒服又安全」的 Benagil
@card: Benagil 舒適安全版：先挑船型與出發地（短程、好上下船）比挑旅行社名字更重要；上午通常較穩，下午起風就改陸上觀景，別把行程押在洞穴上。
...
### cost
@card: 花費抓法：不估死價格，先用船型/出發地/季節決定；小船導覽通常最省體力也最符合「看洞」需求，Kayak/SUP 更吃海況與體力。
費用差異很大（船型、季節、出發地、是否含裝備/保險），所以不在這裡估死價格。抓概念：
- **小船導覽**：通常最省體力、也最符合「只想看洞」的需求
- **導覽 kayak/SUP**：比較吃體力與海況（也更看當天風浪）

### backup
@card: Plan B：不要把 Benagil 當成必做；浪況不佳就改陸上 Marinha / Algar Seco 一樣很值得，把成功率放在「風景」而不是「一定要進洞」。
不要把 Benagil 當成「一定要進洞」：浪況不佳就改成 Praia da Marinha / Algar Seco 這種陸上景點，體驗一樣很值得。

### sources
@card: 資料來源與查詢入口：2024-08 新規與限制整理、以及出發地/業者範例連結在這裡，想核對規則可從這段往下追。
