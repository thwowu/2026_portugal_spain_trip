...
import { ITINERARY_PHASES } from '../../data/itinerary'
...
import { inlineToPlainText } from '../../markdownLite/inline'
...
const ITINERARY_CITY_ORDER: CityId[] = computeItineraryCityOrder({
  itineraryPhases: ITINERARY_PHASES,
  extraCityIds: ATTRACTIONS_DATA.map((c) => c.cityId as CityId),
  fallbackOrder: [...(STAYS_CITY_ORDER as unknown as CityId[])],
})

// Keep Attractions city order aligned with the itinerary.
const DEFAULT_ATTRACTIONS_CITY_ID: CityId | null = computeDefaultAttractionsCityId({
  itineraryCityOrder: ITINERARY_CITY_ORDER,
  attractionsCityIds: ATTRACTIONS_DATA.map((c) => c.cityId as CityId),
})
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        const section = document.querySelector<HTMLElement>(
          `section.attrCitySection[data-city-id="${activeCityId}"]`,
        )
    ...
    <div className="container pageAttractions">
          ...
          <div className="cardInner attrSectionProgressInner">
            <div className="attrSectionProgressTopRow">
              <div className="attrSectionProgressLabel">
                ...
                <span className="attrSectionProgressCity">
                  {activeCityId ? CITIES[activeCityId]?.label : '—'}
                </span>
                <span className="attrSectionProgressMeta">
                  {activeCityId ? `${activeCityIdx + 1}/${cityOrder.length}` : '—'}
                </span>
              </div>
            ...
            <div className="attrSectionProgressBar" aria-hidden="true">
              <div
                className="attrSectionProgressBarFill"
                style={{ width: `${Math.round(cityProgress * 100)}%` }}
              />
            </div>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gap: 14 }}>
        {orderedAttractions.map((c) => {
          ...
          const activeSection =
            c.sections.find((s) => s.kind === activeKind) ?? visibleSections[0] ?? c.sections[0]
          const sectionContent = activeSection?.content ?? ''
          const cleanedSectionContent = stripCardLinesFromContent(sectionContent)
          const canOpenLongRead =
            !!cleanedSectionContent.trim() &&
            cleanedSectionContent.trim().length >= LONG_SECTION_MODAL_THRESHOLD_CHARS
                        ...
                        {!cleanedSectionContent.trim() ? (
                          ...
                          <>
                            {activeSection.kind === 'routes' ? (
                              ...
                              <>
                                {(() => {
                                  const rawItems = extractH3CarouselItems(sectionContent, { snippetMaxLen: 140 })
                                  // The authored content often starts with "### Key takeaways" (a summary list).
                                  // In the Attractions carousel, we want the first card to be an actual attraction.
                                  const items = rawItems.filter((it) => {
                                    const s = (it.title ?? '').trim().toLowerCase()
                                    return !s.includes('key takeaways')
                                  })
                                ...
                                {extractH3CarouselItems(sectionContent).length < 3 ? (
                                  <div style={sectionHueVars(activeSection.kind)}>
                                    <RichContent
                                      content={cleanedSectionContent}
                          ...
                          {(
                            ...
                            return (
                              <button
                                ...
                                onClick={() =>
                                  setLongRead({
                                    ariaLabel: `${c.title}｜${inlineToPlainText(t.title)}`,
                                    headerTitle: t.title,
                                    headerSub: c.title,
                                    content,
                                    kind: t.kind,
                                    ...
                                    <div key={t.id} className="card" style={{ boxShadow: 'none', marginTop: 10 }}>
                                      <div className="cardInner">
                                        ...
                                        <div style={{ marginTop: 10 }}>
                                          <button
                                            className="btn"
                                            onClick={() => setActive({ cityId: c.cityId, tripId: t.id })}
                                          >
                                            詳情…
                                          </button>
                                        </div>
      ...
      {longRead && (
        ...
        >
          <div style={sectionHueVars(longRead.kind)}>
            <RichContent
...
function RevealSection({
  ...
  return (
    <section
      ...
      data-city-id={cityId}
      data-testid={testId}
      className="reveal attrCitySection"
      ref={ref}
    >
      {children}
    </section>
